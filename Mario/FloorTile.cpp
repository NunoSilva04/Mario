...
IGameObjects::~IGameObjects() {

}

void FloorTile::initInstanceBuffer(ID3D11Device* dev) {
	HRESULT hr;
	...
	for (; index < numInstancesToCreate; index++) {
		...
		tempInstance.newUVValues[0] = DirectX::XMFLOAT2(0.0f, 0.0f);
		tempInstance.newUVValues[1] = DirectX::XMFLOAT2(0.142857f, 0.0f);
		tempInstance.newUVValues[2] = DirectX::XMFLOAT2(0.0f, 0.33333f);
		tempInstance.newUVValues[3] = DirectX::XMFLOAT2(0.142857f, 0.33333f);
	...
	//2nd part
	numInstancesToCreate += 30; offsetX += 1.5f; offsetY = 0.0f;
	for (; index < numInstancesToCreate; index++) {
		if (index == 165) {
			offsetX -= 7.5f;
			offsetY -= 0.5f;
	...
	//3rd part
	numInstancesToCreate += 120; offsetX += 1.5f; offsetY = 0.0f;
	for (; index < numInstancesToCreate; index++) {
		if (index == 240) {
			offsetX -= 30.0f;
			offsetY -= 0.5f;
	...
	//4th part
	numInstancesToCreate += 110; offsetX += 1.5f; offsetY = 0.0f;
	for (; index < numInstancesToCreate; index++) {
		if (index == 355) {
			offsetX -= 27.5f;
			offsetY -= 0.5f;
...
const Microsoft::WRL::ComPtr<ID3D11Buffer> FloorTile::getInstanceBuffer() const {
	return instanceBuffer;
}

const UINT FloorTile::getSizeofInstanceStruct() const {
	return sizeof(instanceStruct);
}

const int FloorTile::getNumBlockTypeInstances() {
	return instances.size();
}

const int FloorTile::getNumBlockTypeInstancesToRender() const {
	return instancesToRender.size();
}

const DirectX::XMFLOAT3 FloorTile::getInstanceVertice(int ID, int verticeNumber) {
	return instances[ID].verticesPos[verticeNumber];
}

void FloorTile::resetInstances() {
	instancesToRender = std::vector<instanceStruct>();
}

void FloorTile::addIstance(int instanceID) {
	instanceStruct newInstance;
	newInstance.instanceID = instanceID;
	newInstance.newUVValues[0] = DirectX::XMFLOAT2(0.0f, 0.0f);
	newInstance.newUVValues[1] = DirectX::XMFLOAT2(0.142857f, 0.0f);
	newInstance.newUVValues[2] = DirectX::XMFLOAT2(0.0f, 0.33333f);
	newInstance.newUVValues[3] = DirectX::XMFLOAT2(0.142857f, 0.33333f);
...
void FloorTile::updateBlockTypeInstanceBuffer(ID3D11DeviceContext *devCon) {
	HRESULT hr;
...
void FloorTile::getVerticesPosition(DirectX::XMFLOAT3* positions, int currInstance) const {
